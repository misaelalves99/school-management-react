...
export const mockSubjects = {
  list: (): Subject[] => [...subjectsData], // sempre retorna cópia
  ...
  create: (subject: Omit<Subject, 'id'>): Subject => {
    ...
    subjectsData = [...subjectsData, newSubject]; // atualiza criando novo array
    return newSubject;
  },
  ...
  update: (id: number, updated: Partial<Omit<Subject, 'id'>>): Subject | undefined => {
    ...
    subjectsData = subjectsData.map(s => s.id === id ? updatedSubject : s); // cria novo array
  ...
  delete: (id: number): boolean => {
    ...
    subjectsData = subjectsData.filter(s => s.id !== id); // cria novo array
    return true;
  }
};
